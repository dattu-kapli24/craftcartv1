import { getStoreData, saveStoreConfig, onAuthChange, logoutAdmin, getStoreIdFromUrl, getOwnedStores, syncAllBlueprintsToFirestore, auth } from "./firebase-service.js";
...
(async function() {
  ...
  // Close sidebar when clicking outside on mobile (using the overlay created in CSS)
  ...
  // SYNC ALL BLUEPRINTS & IMAGES TO FIRESTORE
  if ($("syncAllBlueprintsBtn")) {
    $("syncAllBlueprintsBtn").onclick = async () => {
      if (!confirm("This will synchronize all store blueprints and canonical product images across all catalogs to your Firebase database. Proceed?")) return;
      const btn = $("syncAllBlueprintsBtn");
      btn.disabled = true;
      btn.textContent = "Syncing images & catalogs to Firebase...";
      try {
        const res = await syncAllBlueprintsToFirestore();
        if (res.success) {
          alert(`Successfully synced ${res.syncedStores.length} stores (all product images updated) to Firebase!`);
          window.location.reload();
        } else {
          alert("Error syncing to Firebase: " + (res.error || "Unknown error"));
        }
      } catch (err) {
        alert("Sync error: " + err.message);
      } finally {
        btn.disabled = false;
        btn.textContent = "🔄 Sync All Product Images & Catalogs to Database";
      }
    };
  }
